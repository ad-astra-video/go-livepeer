...
func (bsm *BroadcastSessionsManager) selectSessionsManual(ctx context.Context, orchAddr string, orchUrl string) (bs []*BroadcastSession, calcPerceptualHash bool, verified bool) {
	bsm.sessLock.Lock()
	defer bsm.sessLock.Unlock()
...
func transcodeSegment(ctx context.Context, cxn *rtmpConnection, seg *stream.HLSSegment, name string,
	...
	if orchAddr != "" {
		...
		sessions, calcPerceptualHash, verified = cxn.sessManager.selectSessionsManual(ctx, orchAddr, orchUrl)
	} else {
