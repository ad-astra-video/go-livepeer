...
func (bsm *BroadcastSessionsManager) selectSessionsManual(ctx context.Context, orchAddr string, orchUrl string) (bs []*BroadcastSession, selectedReasons []string, calcPerceptualHash bool, verified bool) {
	bsm.sessLock.Lock()
	defer bsm.sessLock.Unlock()
	clog.V(common.DEBUG).Infof(ctx, "Searching %v trusted and %v untrusted seesions for orchAddr=%v orchUrl=%v", len(bsm.trustedPool.sessMap), len(bsm.untrustedPool.sessMap), orchAddr, orchUrl)
...
func transcodeSegment(ctx context.Context, cxn *rtmpConnection, seg *stream.HLSSegment, name string,
	...
	if orchAddr != "" {
		...
		orchUrl := ""
		if strings.Contains(orchAddr, "|") {
			orchAddrSplit := strings.Split(orchAddr, "|")
			orchAddr = orchAddrSplit[0]
			orchUrl = orchAddrSplit[1]
		}

		sessions, selectedReasons, calcPerceptualHash, verified = cxn.sessManager.selectSessionsManual(ctx, orchAddr, orchUrl)
	} else {
