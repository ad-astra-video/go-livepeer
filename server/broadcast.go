	...
	for _, session := range sessions {
		...
		if len(session.SegsInFlight) == 0 {
			if session.LatencyScore > 0 && session.LatencyScore <= SELECTOR_LATENCY_SCORE_THRESHOLD {
				session.SelectedBy = fmt.Sprintf("performance: no segments in flight, latency score of %v < %v", session.LatencyScore, durMult)
				return session
			}
		}
		...
		if len(session.SegsInFlight) > 0 {
			...
			if timeInFlight < maxTimeInFlight {
				session.SelectedBy = fmt.Sprintf("performance: segments in flight, latency score of %v < %v", session.LatencyScore, durMult)
				return session
			}
		}
	}
