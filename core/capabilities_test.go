...
func TestCapabilities_CapabilityConstraints(t *testing.T) {
	...
	//add another model
	caps.constraints.AddConstraints(Capability_TextToImage, &Constraints{Models: newModelConstraint.Models})
	...
	caps.constraints.AddConstraints(Capability_TextToImage, newModelConstraint)
	checkCapsConstraints = caps.constraints
	...
	caps.constraints.RemoveConstraints(Capability_TextToImage, newModelConstraint)
...
func (os *stubOS) Presign(url string, dur time.Duration) (string, error) {
